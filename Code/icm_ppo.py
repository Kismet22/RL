...
class ICM(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_layer_dim, scale, bias):
        super().__init__()
    ...
    def icm_forward(self, actions_in, states_in, next_states_in, icm_alpha):
        # normalize
        ...
        icm_rewards = icm_alpha * icm_forward_loss.mean(dim=-1)  # [batch_size, t]
...
class ICM_PPO:
    def __init__(self, state_dim, action_dim, lr_actor, lr_critic,
        ...
        self.icm = ICM(state_dim=state_dim, action_dim=action_dim, hidden_layer_dim=hidden_layer_dim,
                       scale=continuous_action_output_scale, bias=continuous_action_output_bias).to(device)
    ...
    def update(self):
        ...
        with torch.no_grad():
            ...
            _, _, icm_rewards = self.icm.icm_forward(actions_in=old_actions_detached, states_in=old_states_detached,
                                                     next_states_in=old_next_states_detached, icm_alpha=icm_alpha)
